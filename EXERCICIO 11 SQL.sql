...
alter table tbProduto_Historico add Ocorrencia VARCHAR (200) null;
alter table tbProduto_Historico add Atualizacao datetime null;

-- exercicio 18

alter table tbProduto_Historico drop primary key;
alter table tbProduto_Historico add constraint PK_Id_tbProdutoHistorico primary key (CodigoBarras, Ocorrencia, Atualizacao);

describe tbProduto_Historico;

-- exercicio 19

DELIMITER $$
create trigger TRG_ProdutoHistoricoInsert after insert on tbProduto for each row
begin
	insert into tbProduto_Historico
...
(12345678910119, 'Agua Mineral', 1.99, 500);

select * from tbProduto_Historico;

-- exercicio 20

DELIMITER $$

create trigger TRG_ProdutoHistoricoUptade after update on tbProduto for each row
begin
	insert into tbProduto_Historico
		set CodigoBarras = new.CodigoBarras,
			Nome = new.Nome,
...
update tbProduto set ValorUnitario = 2.99 where CodigoBarras = 12345678910119;
